...
# group:    integer vector containing the group of the observations. Default is
#           the third column of the values, the groups drawn by data_gen
wrap_sim <- function(values, group = values[, 3], t_mat_list, hyp_list, variant){
...
print(t_mat_list)
  for( hyp in hyp_list){
    t_mat <- t_mat_list[[hyp]]
    erg_int[[hyp]] <- test_stat(values, t_mat_list[[hyp]], var_out = var_int)
  }
 print(erg_int)

  out <- c( unlist(erg_int))
  names(out) <- c( paste0("int","_" , hyp_list))
  return(out)
}
